...
export default function Papers({ navigate }) {
  ...
  const fetchPapers = () => {
    ...
    setLoading(true)
    axios.get('/api/papers', { params }).then(res => setPapers(res.data))
  ...
  useEffect(() => { fetchPapers() }, [subjectFilter, yearFilter, examTypeFilter])
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="loading"><div className="spinner"></div><p>Loading papers...</p></div>
      ) : papers.length === 0 ? (
        <div className="empty-state">
          <h3>No papers found</h3>
          <p>Try changing the filters or add papers from the Subjects page.</p>
          <button onClick={() => navigate('subjects')} className="btn btn-primary" style={{ marginTop: '1rem' }}>Browse Subjects</button>
        </div>
      ) : (
              ...
              <div className="paper-info">
                ...
                {paper.file_url && (
                  <div style={{ marginTop: '0.4rem' }}>
                    <a href={paper.file_url} target="_blank" rel="noopener noreferrer" className="btn btn-outline btn-sm">📄 Download</a>
                  </div>
                )}
              </div>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <button onClick={() => navigate('subject', { id: paper.subject_id })} className="btn btn-outline btn-sm">Subject</button>
